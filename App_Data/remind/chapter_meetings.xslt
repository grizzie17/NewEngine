...
cdata-section-elements="subject content location" />
...
<xsl:template match="event[category='al-i']">
<xsl:copy>
<xsl:apply-templates select="@*|node()"  />
</xsl:copy>
</xsl:template>

<xsl:template match="style[parent::event/category='al-i']">
<style>RmdChapterNeighborSpecial</style>
</xsl:template>


<xsl:template match="category[.='al-i']">
<category>email</category>
<xsl:copy>
<xsl:apply-templates select="@*|node()"  />
</xsl:copy>
